...
from movieapp.models import Cinema, User, Showtime, Movie, Genre, MovieFormat, Room, Province, TranslationType, Ticket, \
    BookingStatus, ShowtimeSeat, SeatStatus, Booking, Seat, SeatType
...
def sample_basic_setup(test_session):
    ...
    entities = [p_hcm, p_hn, f2d, f3d, st_normal, st_vip]
    ...
    for entity in entities: test_session.delete(entity)
    test_session.commit()


# ==========================================
# TẦNG 2: RẠP & PHÒNG & GHẾ
# ==========================================
@pytest.fixture
def sample_cinemas(test_session, sample_basic_setup):
    ...
    for room in [r1, r2, r3]:
        for i in range(1, 4):
            seat_type_id = st_vip.id if i == 2 else st_normal.id
            seats.append(Seat(room_id=room.id, seat_number=f"A0{i}", row="A", col=i, seat_type_id=seat_type_id))
    test_session.add_all(seats)
    test_session.commit()

    # GỘP DỮ LIỆU TẦNG 1 VÀO TẦNG 2
    yield {
        **sample_basic_setup,
    ...
    for obj in seats + [r1, r2, r3, c1, c2, c3]:
...
def sample_users(test_session):
    ...
    # Bọc trong key "users" để khi gộp không bị lộn xộn
    yield {"users": {"user1": u1, "user2": u2}}

    test_session.delete(u1)
    test_session.delete(u2)
    test_session.commit()


# ==========================================
# TẦNG 4: THỂ LOẠI & PHIM
# ==========================================
@pytest.fixture
def sample_movies_data(test_session, sample_cinemas):
    ...
    # GỘP DỮ LIỆU TẦNG 2 (Đã bao gồm tầng 1) VÀO TẦNG 4
    yield {
        **sample_cinemas,
        "genres": {"action": g_action, "comedy": g_comedy},
        "movies": {"hot": m1, "old": m2, "new": m3}
    }

    for obj in [m1, m2, m3, g_action, g_comedy]: test_session.delete(obj)
    test_session.commit()


# ==========================================
# TẦNG 5: SUẤT CHIẾU & TRẠNG THÁI GHẾ
# ==========================================
@pytest.fixture
def sample_showtimes_complex(test_session, sample_movies_data):
    # Nhờ gộp dữ liệu, bạn có thể truy xuất "formats", "rooms" trực tiếp từ sample_movies_data
    ...
    # GỘP TẦNG 4 VÀO TẦNG 5
    yield {
        **sample_movies_data,
        "showtime": st1,
        "showtime_seats": sts_list
    }

    for sts in sts_list: test_session.delete(sts)
    test_session.delete(st1)
    test_session.commit()
...
def sample_full_chain(test_session, sample_users, sample_showtimes_complex):
    ...
    # GỘP TẤT CẢ (USERS + SHOWTIMES + BOOKING) LẠI VỚI NHAU
    yield {
        **sample_users,
        **sample_showtimes_complex,
        "booking": booking,
        "ticket": ticket
    }

    test_session.delete(ticket)
    test_session.delete(booking)
    test_session.commit()
