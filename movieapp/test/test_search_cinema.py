...
from movieapp.test.test_base import sample_cinemas,test_session,test_app,sample_basic_setup


def test_all(sample_cinemas):
    actual_cinemas,total=load_cinema()

    assert len(actual_cinemas)==len(sample_cinemas["cinemas"])
...
def test_pagination(sample_cinemas):
    ...
    assert total1 == len(sample_cinemas["cinemas"])
